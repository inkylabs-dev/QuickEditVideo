...
test.describe('Extract-audio page', () => {
  test('should load extract-audio page successfully', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Audio Extractor - Extract Audio from Video Online Free/);
  });
  ...
  test('should have correct page header', async ({ page }) => {
    ...
    await expect(page.locator('text=Extract audio from videos as MP3 or WAV')).toBeVisible();
  });
  ...
  test('should navigate back to home page', async ({ page }) => {
    ...
    // Find and click a home/back navigation link
  ...
  test('should extract audio and download file', async ({ page }) => {
    ...
    try {
      ...
      // Wait for FFmpeg to load (indicated by extract button being enabled)
      await page.waitForSelector('button:has-text("Extract As"):not([disabled])', { timeout: 30000 });
