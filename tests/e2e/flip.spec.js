...
test.describe('Flip page - E2E', () => {
  ...
  test('should flip video horizontally and download file', async ({ page }) => {
    ...
    // Wait for download button to be enabled (no FFmpeg loading required with MediaBunny)
    ...
    expect(download.suggestedFilename()).toMatch(/.*_flipped\.(mp4|mov|webm|mkv)$/);
  ...
  test('should show download button when video is uploaded', async ({ page }) => {
    await page.goto('/flip');
    ...
    // Should show download button (MediaBunny doesn't require loading like FFmpeg)
    await expect(page.locator('button:has-text("Download")')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should handle different video formats', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ hasText: 'Download' })).toBeVisible({ timeout: 10000 });
