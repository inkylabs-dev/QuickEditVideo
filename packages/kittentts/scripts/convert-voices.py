...
def main():
    ...
    npz_path = os.path.join(assets_dir, 'voices.npz')
    json_path = os.path.join(assets_dir, 'voices.json')
    
    if not os.path.exists(npz_path):
        print(f"Error: {npz_path} not found. Please run 'npm run download-model' first.")
        sys.exit(1)
