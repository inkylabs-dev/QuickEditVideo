...
const VideoWatermarkContent = () => {
	...
	const processWatermark = async () => {
		if (!ffmpegLoaded || !videoFile || !logoFile) {
				...
				<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
					<div class="text-center mb-8">
						<div class="w-16 h-16 bg-teal-100 rounded-full flex items-center justify-center mx-auto mb-4">
							<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-teal-600">
								<rect width="18" height="18" x="3" y="3" rx="2"/>
								<path d="M9 9h6v6H9z"/>
								<path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/>
							</svg>
						</div>
						<h2 class="text-2xl font-semibold text-gray-900 mb-2">Add Watermark to Video</h2>
						<p class="text-gray-600">Upload your video and logo to get started</p>
					</div>

					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						{/* Video Upload */}
						<div 
							class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-teal-400 hover:bg-teal-50 transition-colors cursor-pointer"
						...
						>
							...
							<h3 class="font-medium text-gray-900 mb-2">Upload Video</h3>
							<p class="text-sm text-gray-600 mb-4">MP4, WebM, AVI, MOV and more</p>
							{videoFile ? (
								<div class="text-sm text-teal-600 font-medium">✓ {videoFile.name}</div>
							) : (
								<div class="text-sm text-gray-500">Click to browse or drag & drop</div>
							)}
							<input
						...
						>
							...
							<h3 class="font-medium text-gray-900 mb-2">Upload Logo</h3>
							<p class="text-sm text-gray-600 mb-4">PNG, JPG, SVG and more</p>
							{logoFile ? (
								<div class="text-sm text-teal-600 font-medium">✓ {logoFile.name}</div>
							) : (
								<div class="text-sm text-gray-500">Click to browse or drag & drop</div>
							)}
							<input
			...
			<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
				...
				<div class="p-6 border-b border-gray-200">
					<div class="flex items-center justify-between">
						<div>
							<h3 class="font-semibold text-gray-900">Watermark Editor</h3>
							<p class="text-sm text-gray-600 mt-1">Position and resize your logo</p>
						</div>
						<button 
							onClick={resetFiles}
				...
				<div class="p-6">
					<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
						...
						<div class="lg:col-span-2">
							<div class="relative bg-black rounded-lg overflow-hidden video-container-custom">
								{videoUrl && (
									<video
										ref={videoRef}
										src={videoUrl}
								...
								{logoUrl && originalWidth > 0 && (
									<div 
										class="absolute border-2 border-teal-500 cursor-move"
									...
									>
										...
										{/* Resize handle */}
										<div 
											class="absolute bottom-0 right-0 w-3 h-3 bg-teal-500 cursor-se-resize"
											onMouseDown={(e) => handleResizeStart(e, 'bottom-right')}
											onTouchStart={(e) => handleResizeStart(e, 'bottom-right')}
										></div>
										
										{/* Center label with drag/resize hint */}
										<div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-teal-600 text-white px-2 py-1 rounded text-xs font-medium pointer-events-none flex items-center gap-1">
											{isDragging ? (
												<>
						...
						<div class="space-y-6">
							...
							<div class="space-y-4">
								<div>
									<h3 class="font-semibold text-gray-900">Position Controls</h3>
									<div class="grid grid-cols-2 gap-3 mt-3">
										<div>
											<label class="block text-sm font-medium text-gray-700 mb-1">Left (px)</label>
											<input
												type="number"
												value={logoLeft}
										...
										<div>
											<label class="block text-sm font-medium text-gray-700 mb-1">Top (px)</label>
											<input
												type="number"
												value={logoTop}
								...
								{/* Size Controls */}
								<div>
									<h3 class="font-semibold text-gray-900">Size Controls</h3>
									<div class="grid grid-cols-2 gap-3 mt-3">
										<div>
							...
							<div class="border-t border-gray-200 pt-6">
								<button
									...
									class={`w-full py-3 px-4 rounded-lg font-medium text-center transition-colors ${
								...
								{!ffmpegLoaded && (
									<p class="text-sm text-gray-500 mt-2 text-center">Loading FFmpeg...</p>
								)}
							</div>
